...
from sklearn.metrics import confusion_matrix, classification_report, precision_score, recall_score, f1_score, roc_curve, \
    roc_auc_score, accuracy_score, classification_report, confusion_matrix, auc
...
.stGraph>div>div>div>canvas {
    border-radius: 5px !important;
    border: 1px solid #dfe6e9 !important;
}

.stGraph>div>div>div>div {
    border-radius: 5px !important;
}

.stGraph>div>div>div>div>div {
    border-radius: 5px !important;
}

.stGraph>div>div>div>div>div>div {
    border-radius: 5px !important;
}
...
if st.sidebar.checkbox('Compare algorithms'):
    ...
    def visualize_confusion_matrix(y_test, y_pred):
        ...
        sns.heatmap(cm, annot=True, fmt='g', cmap='Oranges',
                    ...
                    yticklabels=['No Credit Card Fraud Dection', 'Credit Card Fraud Dection'], ax=ax)
        ax.set_title('Accuracy: {0:.4f}'.format(accuracy_score(y_test, y_pred)))
    ...
    def ROC_AUC(Y, Y_prob):
        ...
        ax.plot(fpr, tpr, marker='.', label='Model - AUC=%.3f' % (model_auc))
    ...
    confusion_matrix_fig_train = visualize_confusion_matrix(y_train, y_train_pred)
    st.pyplot(confusion_matrix_fig_train)
    # Display ROC curve for training set
    roc_auc_fig_train = ROC_AUC(y_train, y_train_pred)
    st.pyplot(roc_auc_fig_train)
